...
<template>
  <div class="w-full mb-8">
    <div class="relative hidden sm:block">
      <!-- Desktop Timeline -->
      <div class="h-1 bg-gray-700 w-full absolute top-6"></div>
      <div
        class="h-1 bg-primary/70 absolute top-6 transition-all duration-500"
          ...
          <div
            :class="[
              ...
              currentStageIndex >= index
                ...
                : 'bg-gray-700',
              currentStageIndex === index && 'animate-pulse ring-4 ring-primary/30',
            ]"
          >
            <component
              :is="stage.icon"
              :class="[
                'w-6 h-6',
                currentStageIndex >= index ? 'text-white' : 'text-gray-400',
              ]"
            />
          </div>
          <span
            :class="[
              'text-sm font-medium',
              currentStageIndex >= index ? 'text-primary' : 'text-gray-400',
            ]"
            >{{ stage.title }}</span
          >
          <span
            :class="[
              'text-xs',
              currentStageIndex >= index ? 'text-gray-300' : 'text-gray-500',
            ]"
            >{{ stage.subtitle }}</span
          >
        </div>
      ...
      <div class="flex flex-col space-y-4">
        <div
          ...
          :class="[
            ...
            currentStageIndex === index ? 'opacity-100' : 'opacity-70',
          ]"
        >
          <div
            :class="[
...
import { CircleCheck, Circle, Supernova, Download } from "lucide-vue-next";
...
const props = defineProps({
  currentStage: {
    ...
    required: true,
    validator: (value) => {
      return ["created", "payment", "processing", "completed"].includes(value);
    },
  },
});
