...
const selectedCountry = computed(() => {
  return countries.value.find(country => country.dial_code === props.modelValue) || countries.value[0];
});
...
<template>
  <div class="country-selector relative mb-4">
    ...
    <div id="country-dropdown" class="relative mt-1">
      <button 
        ...
        class="cosmic-input flex items-center justify-between w-full px-4 py-2 text-white bg-dark-lighter border border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
      >
      ...
      <div 
        v-show="isOpen"
        class="absolute z-10 w-full mt-1 overflow-auto bg-dark-lighter border border-gray-700 rounded-md shadow-lg max-h-60"
      >
        <!-- Search input -->
        <div class="sticky top-0 px-3 py-2 bg-dark-lighter">
          <input
            ...
            @input="handleSearch"
            class="w-full px-2 py-1 cosmic-input text-white rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
          />
        </div>
        ...
        <ul class="py-1">
          ...
          >
            ...
            <span class="ml-auto text-gray-400">{{ country.dial_code }}</span>
          </li>
        </ul>
      </div>
    </div>
