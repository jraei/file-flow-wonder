...
const selectAllOnPage = () => {
    ...
    const allSelected = currentIds.every(id => selectedProducts.value.includes(id));
    
    if (allSelected) {
        // Unselect all on current page
        selectedProducts.value = selectedProducts.value.filter(id => !currentIds.includes(id));
    } else {
        // Select all on current page
        const newSelected = [...new Set([...selectedProducts.value, ...currentIds])];
...
const submitBulkAssign = async () => {
    ...
    try {
        ...
        successMessage.value = response.data.message || `${selectedProducts.value.length} products have been assigned.`;
        ...
        errorMessage.value = error.response?.data?.message || 'Failed to assign products.';
    } finally {
...
<template>
    <Modal :show="show" @close="close" max-width="5xl">
        <div class="relative border border-gray-700 rounded-lg p-3 sm:p-4 md:p-6 bg-dark-card max-h-[85vh] overflow-hidden">
            <!-- Cosmic Background -->
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-xl font-bold text-white flex items-center space-x-2">
                        ...
                        <span v-if="hasSelectedProducts" class="ml-2 px-2.5 py-0.5 text-xs bg-secondary/20 text-secondary border border-secondary/30 rounded-full flex items-center">
                            {{ selectedCount }} selected
                            <span v-if="!isLowPowerDevice" class="comet-trail"></span>
                        </span>
                    </h3>
                ...
                <div class="mb-4 p-3 rounded-lg bg-dark-lighter border border-primary/30 animate-fade-in">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <span v-if="category?.kode_kategori" class="px-2 py-0.5 bg-orange-500/20 text-orange-400 text-xs rounded-full border border-orange-500/30 nebula-pulse">
                ...
                <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 mb-4">
                    ...
                    <button
                        ...
                        class="px-4 py-2 rounded-lg flex items-center justify-center space-x-2 border transition-all"
                        :class="[
                            isAllOnPageSelected
                    ...
                    >
                        ...
                        <span>{{ isAllOnPageSelected ? 'Deselect All' : 'Select All' }}</span>
                    </button>
                </div>
                ...
                <div class="overflow-y-auto max-h-[50vh] cosmic-scrollbar pr-1">
                    ...
                    <div v-else-if="products.length === 0" class="py-12 text-center">
                        <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-dark-sidebar mb-4">
                            <span class="text-2xl">📦</span>
                        </div>
                        <h4 class="text-lg font-medium text-white">No products found</h4>
                    ...
                    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        <div
                            ...
                            :key="product.id"
                            class="product-card p-3 rounded-lg border cursor-pointer transition-all relative overflow-hidden"
                            :class="[
                        ...
                        >
                            <!-- Cosmic glow effect -->
                            <div 
                                v-if="selectedProducts.includes(product.id) && !isLowPowerDevice" 
                            ...
                            <div class="relative z-10 flex">
                                <!-- Product Image -->
                                <div class="flex-shrink-0 w-16 h-16 mr-3 rounded-md bg-dark-sidebar border border-gray-700/50 flex items-center justify-center overflow-hidden">
                                    <img
                                ...
                                <!-- Product Info -->
                                <div class="flex-grow">
                                    <h4 class="font-medium text-white line-clamp-1">{{ product.nama }}</h4>
                                    
                                    <!-- Current Category -->
                                    <div class="flex items-center mt-2">
                                        <span class="text-xs text-gray-400 mr-1">Category:</span>
                                        <span
                                ...
                                <!-- Selection Indicator -->
                                <div v-if="selectedProducts.includes(product.id)" class="absolute top-2 right-2 h-5 w-5 text-primary">
                            ...
                            <div v-if="selectedProducts.includes(product.id) && !isLowPowerDevice" class="quantum-lines"></div>
                        </div>
                    </div>
                    
                    <!-- Pagination -->
                    <div v-if="pagination.lastPage > 1" class="mt-4 flex justify-center space-x-2">
                        <button
                ...
                <div class="mt-6 flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-2">
                    ...
                    >
                        ...
                        <span>{{ isSubmitting ? 'Assigning Products...' : 'Assign Selected Products' }}</span>
                    </button>
                </div>
            </div>
...
.supernova-explosion {
    ...
    width: 150px;
    height: 150px;
    background: radial-gradient(circle, rgba(249, 115, 22, 0.8) 0%, rgba(249, 115, 22, 0.1) 70%, transparent 100%);
    border-radius: 50%;
...
@media (prefers-reduced-motion: reduce) {
    .quantum-lines, .cosmic-glow, .comet-trail, .nebula-pulse, .wormhole-effect {
        animation: none;
    }
