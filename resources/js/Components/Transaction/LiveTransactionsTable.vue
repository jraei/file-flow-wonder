...
<template>
    <div class="relative mt-8 overflow-hidden rounded-xl shadow-lg backdrop-blur-sm">
        <!-- Animated asteroid belt border -->
        <div class="absolute -inset-[1px] bg-gradient-to-r from-primary via-secondary to-primary opacity-70 rounded-xl animate-border-flow"></div>
        
        <div class="relative overflow-x-auto bg-dark-card/90 rounded-xl">
            <table class="w-full text-sm text-left text-gray-300">
                <thead class="text-xs uppercase bg-dark-lighter/50 text-gray-200">
                    <tr>
                        <th scope="col" class="px-2 py-3 sm:px-4">Date</th>
                        <th scope="col" class="px-2 py-3 sm:px-4 hidden sm:table-cell">Invoice</th>
                        <th scope="col" class="px-2 py-3 sm:px-4 hidden md:table-cell">Game</th>
                        <th scope="col" class="px-2 py-3 sm:px-4 hidden sm:table-cell">Price</th>
                ...
                <tbody>
                    <template v-if="transactions.data && transactions.data.length > 0">
                        <tr 
                            ...
                            :key="transaction.id" 
                            class="border-b border-gray-700/30 hover:bg-dark-lighter/30 transition-colors"
                            :class="getRowAnimationClass(index)"
                        >
                            <td class="px-2 py-2 sm:px-4 sm:py-3">{{ formatDate(transaction.created_at) }}</td>
                            <td class="px-2 py-2 sm:px-4 sm:py-3 hidden sm:table-cell">{{ transaction.masked_order_id }}</td>
                            <td class="px-2 py-2 sm:px-4 sm:py-3 hidden md:table-cell">
                                <div class="flex items-center gap-2">
                                    <div v-if="transaction.layanan?.produk?.thumbnail" class="h-6 w-6 overflow-hidden rounded-full bg-dark-lighter">
                                        <img :src="transaction.layanan.produk.thumbnail" class="h-full w-full object-cover" :alt="transaction.layanan.produk.nama" />
                                    </div>
                                    <span v-if="transaction.layanan?.produk?.nama" class="truncate max-w-[150px]">
                                        {{ transaction.layanan.produk.nama }}
                                    </span>
                                    <span v-else>-</span>
                                </div>
                            </td>
                            <td class="px-2 py-2 sm:px-4 sm:py-3 hidden sm:table-cell">{{ transaction.masked_price }}</td>
                    ...
                    <tr v-else class="border-b border-gray-700/30">
                        <td colspan="5" class="px-4 py-8 text-center text-gray-400">
                            <div class="flex flex-col items-center">
                                <!-- Empty state with constellation pattern -->
                                <div class="relative h-32 w-32 mb-4">
                                    <div class="absolute inset-0 flex items-center justify-center">
                                        <div class="h-20 w-20 rounded-full bg-dark-lighter/50 flex items-center justify-center">
                                    ...
                                    <!-- Constellation dots -->
                                    <div class="absolute top-1/4 left-1/4 h-1 w-1 bg-primary rounded-full"></div>
                                    <div class="absolute top-3/4 left-1/2 h-1 w-1 bg-secondary rounded-full"></div>
                                    <div class="absolute top-1/2 left-3/4 h-1 w-1 bg-primary rounded-full"></div>
                                    <div class="absolute top-1/3 left-2/3 h-0.5 w-0.5 bg-white rounded-full animate-pulse"></div>
                                    <div class="absolute top-2/3 left-1/3 h-0.5 w-0.5 bg-white rounded-full animate-pulse"></div>
                                    <!-- Constellation lines -->
                                    <svg class="absolute inset-0 h-full w-full" xmlns="http://www.w3.org/2000/svg">
                                        <line x1="25%" y1="25%" x2="50%" y2="75%" class="stroke-primary/30 stroke-[0.5]" />
                                        <line x1="50%" y1="75%" x2="75%" y2="50%" class="stroke-primary/30 stroke-[0.5]" />
                                        <line x1="75%" y1="50%" x2="25%" y2="25%" class="stroke-secondary/30 stroke-[0.5]" />
                                    </svg>
                                </div>
                                <p class="text-gray-400 mb-1">No transactions found</p>
                                <p class="text-gray-500 text-xs">The cosmic void is peaceful today</p>
                            </div>
                        </td>
                    </tr>
                </tbody>
            ...
            <!-- Pagination -->
            <div v-if="transactions.meta && transactions.meta.last_page > 1" class="flex justify-center py-4 bg-dark-lighter/20">
                <nav class="flex items-center space-x-1">
                    <button 
                        @click="changePage(transactions.meta.current_page - 1)" 
                        :disabled="transactions.meta.current_page === 1"
                        class="px-3 py-1 rounded bg-dark-lighter/50 text-gray-400 hover:bg-dark-lighter disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        Previous
                    </button>
                    
                    <div class="flex items-center space-x-1">
                        <template v-for="page in transactions.meta.last_page" :key="page">
                            <button 
                                v-if="page === 1 || page === transactions.meta.last_page || (page >= transactions.meta.current_page - 1 && page <= transactions.meta.current_page + 1)"
                            ...
                            <span 
                                v-else-if="page === 2 || page === transactions.meta.last_page - 1"
            ...
            <div class="absolute top-3 right-3">
                <div class="flex items-center text-xs text-gray-400">
                    <span class="h-1.5 w-1.5 rounded-full bg-secondary animate-pulse mr-1"></span>
...
const addWarpTunnelEffect = () => {
    const table = document.querySelector('.bg-dark-card');
    if (!table) return;
...
const drawEntanglementLines = (inputId, transactionIds) => {
    const rows = transactionIds.map(id => 
        document.querySelector(`tr[data-transaction-id="${id}"]`)
    ).filter(Boolean);
    ...
    for (let i = 0; i < rows.length - 1; i++) {
        for (let j = i + 1; j < rows.length; j++) {
            ...
            for (let p = 0; p < 2; p++) {
                ...
                particle.setAttribute('fill', p === 0 ? 'rgba(155, 135, 245, 0.7)' : 'rgba(51, 195, 240, 0.7)');
    ...
    document.querySelector('.bg-dark-card').appendChild(svgContainer);
};
...
onMounted(() => {
    ...
    setTimeout(() => {
        if (props.transactions.data) {
            props.transactions.data.forEach(transaction => {
                const row = document.querySelector(`tr[data-key="${transaction.id}"]`);
                if (row) {
