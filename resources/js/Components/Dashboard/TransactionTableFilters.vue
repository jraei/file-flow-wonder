        ...
        dateRangePicker = flatpickr(dateRange, {
            ...
            onChange: (selectedDates) => {
                if (selectedDates.length === 2) {
                    dateStart.value = selectedDates[0].toISOString().split('T')[0];
                    dateEnd.value = selectedDates[1].toISOString().split('T')[0];
                    applyFilters();
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div class="relative starlight-dropdown">
                ...
                <div class="absolute inset-0 pointer-events-none">
                    <div class="absolute w-1 h-1 bg-primary rounded-full opacity-60 animate-pulse" style="top: 25%; left: 10%"></div>
                    <div class="absolute w-1 h-1 bg-secondary rounded-full opacity-60 animate-pulse" style="top: 60%; left: 80%"></div>
                    <div class="absolute w-1 h-1 bg-primary rounded-full opacity-60 animate-pulse" style="top: 80%; left: 30%"></div>
                </div>
            </div>
            ...
            <div class="relative warp-speed-datepicker">
                ...
                <div class="absolute inset-0 overflow-hidden pointer-events-none rounded-lg">
                    <div v-for="i in 5" :key="i" 
                        ...
                        :style="`top: ${20 * i}%; width: 100%; opacity: ${0.2 * i}; transform: translateX(${i % 2 ? -50 : 50}%); animation: warp-speed ${1 + i * 0.5}s linear infinite;`">
                    </div>
                </div>
            </div>
                ...
                >
                    <span class="flex items-center space-x-1">
                        ...
                        <div class="relative w-4 h-4">
                            <!-- Planet Export Icon -->
                            <div class="absolute inset-0 w-4 h-4 border-2 border-white rounded-full cosmic-rotate"></div>
                            <div class="absolute w-1 h-1 bg-white rounded-full cosmic-orbit" style="top: -3px; left: 50%; transform: translateX(-50%);"></div>
                        </div>
                    </span>
                </button>
            </div>
...
<style scoped>
.cosmic-filter-card {
    background: linear-gradient(180deg, rgba(26,31,44,0.8) 0%, rgba(31,41,55,0.6) 100%);
