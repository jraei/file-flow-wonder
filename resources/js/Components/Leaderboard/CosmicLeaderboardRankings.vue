...
<template>
  <div class="relative mx-auto max-w-4xl">
    <!-- Loading overlay -->
    <div v-if="loading" class="absolute inset-0 flex items-center justify-center z-10 bg-dark-DEFAULT/80 backdrop-blur-sm rounded-xl">
      <div class="cosmic-loader">
        <div class="planet"></div>
        <div class="ring"></div>
        <p class="text-primary/80 mt-8">Loading cosmic data...</p>
      </div>
    </div>
    
    <!-- No data state -->
    <div v-if="!loading && rankings.length === 0" class="py-16 text-center">
      <Planet class="w-16 h-16 mx-auto mb-4 text-secondary opacity-30" />
      <p class="text-xl text-gray-400">No cosmic purchases detected yet</p>
      <p class="text-gray-500">Be the first to appear on the leaderboard!</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <CosmicRankCard
          ...
          :class="{
            ...
            'order-1 md:order-2 transform md:-translate-y-4': index === 0,
            'order-3': index === 2
          }"
        />
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" v-if="regularRankings.length > 0">
        <CosmicRankCard
...
import { Planet } from 'lucide-vue-next';
